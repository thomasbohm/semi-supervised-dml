...
class Evaluator_DML():
    def __init__(self, cat=0, dev='cpu'):
        self.cat = cat
        self.dev = dev
    ...
    def evaluate(self, model, dataloader, gallery_dl=None,
        ...
        mode = 'backbone' #self.get_mode(dl_ev_gnn, dataloader)

        if mode != 'backbone' and dataroot != 'in_shop':
    ...
    def predict_batchwise_gnn_inshop(self, gnn, graph_generator, dl_ev_gnn, gallery_X, gallery_T, gallery_P, X, T, P):
        ...
        with torch.no_grad():
            for batch in dl_ev_gnn:
                _, Y, I, P = batch
                print(Y)
                fc7 = torch.stack([X[I[0]]] + [gallery[i-num_query] for i in I[1:]]).to(self.dev)
